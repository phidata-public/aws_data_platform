...
##############################################################################
## This file defines a workflow that downloads daily etf prices from tiingo
## The workflow is part of the tiingo data product
##############################################################################


# Step 1: Define a postgres table named `daily_etf_prices`.
etf_prices_table = PostgresTable(
    name="daily_etf_prices",
    db_conn_id=pg_db_connection_id,
    db_conn_url=dev_db.get_db_connection_url_local(),
)


# Step 2: Build a workflow that loads the etf_prices_table
...
class LoadEtfPricesArgs(TaskArgs):
    # The table to load
    etf_prices_table: PostgresTable = etf_prices_table
    # The tickers table to read for etf tickers
...
@task
def drop_etf_prices(**kwargs) -> bool:
    """
    This task drops daily data before loading, so we dont have duplicates
    """
    args = LoadEtfPricesArgs.from_kwargs(kwargs)
    run_date = args.run_date

    # drop_table_before_load
    if args.drop_table_before_load:
        logger.info(f"Dropping table: {args.etf_prices_table.name}")
        args.etf_prices_table.delete()
    # or drop rows for current date so we dont have duplicates
    else:
        logger.info(f"Dropping data for: {run_date}")
        args.etf_prices_table.run_sql_query(
            f"DELETE FROM {args.etf_prices_table.name} WHERE ds = '{run_date}'"
        )
    return True


# 2.3: Instantiate the task
drop = drop_etf_prices()


# 2.4: Write a task to load daily etf prices
@task
def load_etf_prices(**kwargs) -> bool:

    import pandas as pd
    from tiingo import TiingoClient

    # Get inputs as typed arguments by creating LoadEtfPricesArgs from kwargs
    args: LoadEtfPricesArgs = LoadEtfPricesArgs.from_kwargs(kwargs)
    ...
    if args.run_date is None:
        ...
        return False
    logger.info(f"Loading {args.etf_prices_table.name} for {run_date}")
    ...
    etf_tickers: pd.DataFrame = args.tickers_table.run_sql_query(
        sql_query=f"""
        SELECT ticker
    ...
    logger.info(etf_tickers[:5])
    tickers_list = list(etf_tickers["ticker"])
    logger.info(f"# Tickers: {len(tickers_list)}")

    # Get etf prices and load etf_prices_table
    ...
    for ticker in tickers_list:
        ...
        if rows_in_df >= args.cache_size:
            ...
            write_success = args.etf_prices_table.write_pandas_df(
    ...
    return args.etf_prices_table.write_pandas_df(prices_df, if_exists="append")


# 2.5: Instantiate the task to load etf prices
load_etf_prices = load_etf_prices()

# 2.6: Create a Workflow object and add the tasks
etf_prices = Workflow(
    name="etf_prices",
    tasks=[drop, load_etf_prices],
    graph={
        load_etf_prices: [drop],
    },
    outputs=[etf_prices_table],
)
